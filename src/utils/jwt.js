...
export const generateTokens = (user) => {
    const accessToken = jwt.sign({id: user._id, role: user.role}, env.jwtSecret, {expiresIn: "1h"});  //Genera un token de acceso con su firma
    const refreshToken = jwt.sign({id: user._id}, env.jwtRefreshSecret, {expiresIn: "7d"});           //Genera un token de respaldo con su firma

    return {accessToken, refreshToken};
};

/* //Metodo para verificar el token cuando el cliente intenta acceder al server.
